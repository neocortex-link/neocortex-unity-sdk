...
- **Targeted actions**: each action in a reply is `{ name, targetId }`, where `targetId` is the id of the perceived entity it applies to — so "go to the blue cube, then the red one" arrives as two actions with two different targets. `NeocortexActionQueue` runs a reply's actions in order via registered coroutine handlers (`RegisterAction`), with a **Trigger** setting for WHEN they fire: on response, when speech starts, or after the reply is spoken.
...
- **`NeocortexChatUI` drives group chat too**: assign a `NeocortexGroupDirector` instead of a single agent and the same component runs the whole group loop — text/voice input to the director, every speaker's reply into the shared panel under its own name, thinking indicator and mic handoff per turn. Use ONE Chat UI per scene: one per character prints each reply once per UI.
